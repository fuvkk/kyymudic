...
async def pause_cmd(_, message): 
    ...
    await message.reply_text(f"🎧 __**Voicechat Paused**__\n│\n╰ Music paused by {checking}!")
    
@app.on_message(filters.command(["resume", f"resume@{BOT_USERNAME}", "rs"]))
async def stop_cmd(_, message): 
    if message.sender_chat:
        return await message.reply_text("🔴 __You're an **Anonymous Admin**!__\n│\n╰ Revert back to user account from admin rights.") 
    permission = "can_manage_voice_chats"
    m = await adminsOnly(permission, message)
    if m == 1:
        return
    ...
    if not await is_active_chat(chat_id):
        ...
        await message.reply_text(f"🎧 __**Voicechat Resumed**__\n│\n╰ Music resumed by {checking}!")
...
async def stop_cmd(_, message): 
    ...
    if await is_active_chat(chat_id):
        ...
        await remove_active_chat(chat_id)
        await music.pytgcalls.leave_group_call(chat_id)
        await message.reply_text(f"🎧 __**Voicechat End/Stopped**__\n│\n╰ Music ended by {checking}!") 
    else:
        return await message.reply_text("❌ __**I dont think if something's playing on voice chat**__")
...
async def stop_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        ...
        if is_empty(chat_id):
            await remove_active_chat(chat_id)
            await message.reply_text("❌ __**No more music in Queue**__\n\n**»** `Leaving Voice Chat...`")
            await music.pytgcalls.leave_group_call(chat_id)
            return  
        else:
